...
static void *custom_alloc(void *ctx,size_t n,uint8_t align,uintptr_t ra){
    ...
    if(allocator_mode==4){p=packed_alloc(n);mapped_bytes=packed.mapped;mapped_allocations=packed.maps;}
    else if(use_mapping(n)){size_t bytes=page_round(n);p=mmap(NULL,bytes,PROT_READ|PROT_WRITE,MAP_PRIVATE|MAP_ANON,-1,0);if(p==MAP_FAILED)return NULL;mapped_bytes+=bytes;mapped_allocations++;}
    else p=malloc(n);
...
static void custom_free(void *ctx,void *p,size_t n,uint8_t a,uintptr_t ra){
    ...
    if(allocator_mode==4){packed_free(p,n);mapped_bytes=packed.mapped;}
    else if(use_mapping(n)){size_t bytes=page_round(n);assert(munmap(p,bytes)==0);mapped_bytes-=bytes;}else free(p);
}
...
static void stage(const char *name){
    printf("{\"kind\":\"packed_pool\",\"stage\":\"%s\",\"requested\":%zu,\"mapped\":%zu,\"peak_mapped\":%zu,\"maps\":%zu,\"unmaps\":%zu}\n",name,packed.requested,packed.mapped,packed.peak,packed.maps,packed.unmaps);
...
int main(int argc,char **argv){
    assert(argc>=6&&argc<=8);count=atoi(argv[1]);lines=atoi(argv[2]);varied=atoi(argv[3]);compression=atoi(argv[4]);cap_mib=atoi(argv[5]);int relief=(argc>=7)?atoi(argv[6]):0;allocator_mode=(argc==8)?atoi(argv[7]):0;assert(count>0&&count<=512&&lines>=0&&cap_mib>0&&allocator_mode>=0&&allocator_mode<=4);
