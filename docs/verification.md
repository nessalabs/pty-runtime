...
after-all-observers-detach case, split UTF-8/VT sequences under load, and
completing the macOS matrix, which lost 15 of 130 trials to an `lsof` census
failure in the harness and has no `dominant` evidence at all. For **G2**, G1 first, then real-child
...
| …“pass **concurrent pressure** tests” | **run, and failed** | `crates/infrastructure/tests/process_pressure.rs::simultaneous_producers_deliver_complete_bytes_and_quiet_cancel_remains_live`; `tests/runtime_performance.rs::projected_real_pty_producers` | The largest executed concurrency is **16** real producers: 16 × 128 KiB raw, and 16 × 4 MiB projected. ADR 0004 asks for 64 resident sessions with 16 producers at a **combined 10 MiB/s** as the *controlled reference*, then 128 independent producers and 128-session mixed populations as *primary* scenarios, extending to 500 where the host permits. **That has now been run** — see [Experiment 0005](experiments/0005-g1-concurrent-pressure.md), 26 cases × 5 trials on macOS arm64 and Linux x86_64 at `e01c704`. Correctness passed on both hosts across 245 trials. Latency did not: `ProjectedOutput` p99 misses its 20 ms target on all five Linux trials of **`128-active`**, this gate's headline scenario, and by 13× on `chunk-64` — a *paced* workload at ADR-specified parameters, so the "explicit overload outcome" escape does not apply to it. The misses are worse on an idle Linux box than on a loaded laptop, which rules out host contention; the single-trial macOS misses that vanished on Linux were contention. 500 sessions remain unrun. A candidate fix (chunk batching) has since been measured on Linux and brings `chunk-64` to 0.1 ms, `chunk-1` to 5.5-6.0 ms and `128-active` to 3.5 ms at five repeats — see the follow-up section of Experiment 0005. **This does not change the verdict:** the patch is not on `main`, it re-ran 4 of 26 cases, `capacity-projected` is unaddressed, and macOS has no after-number. |
