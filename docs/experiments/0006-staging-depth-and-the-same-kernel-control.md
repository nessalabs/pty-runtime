...
| --- | ---: | --- |
| `fairness`, `500-sessions`, `after-all-observers-detach`, `overload-outcome`, `reference-state`, `soak-55min`, `stalled-sink` | **9** | re-run on the box with the identity-checked collector |
...
Version 9 requires exactly one retained latency measurement for every boundary a
trial judges, and checks each trial's case label against the workload it
actually ran. Version 8 recomputes every target verdict from the measurement it was drawn
