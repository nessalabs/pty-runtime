...
and accounting intact. Prove the allocator contracts with the native/Rust
fixtures; do not check gate log dumps into git. Status: `docs/verification.md`.
...
judgements covered by native tests; status in `docs/verification.md`.
...
in the native page-admission tests.
...
native regression tests for resize, wrap, wide cells, viewport pin,
page capacity, continuation, and page admission. The earlier cursor-only patch
